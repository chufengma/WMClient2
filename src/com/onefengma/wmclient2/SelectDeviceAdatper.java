...
public class SelectDeviceAdatper extends BaseExpandableListAdapter implements OnCheckedChangeListener {
	
	private List<WMDeviceInfo> infos;
	private LayoutInflater inflater;
	private CompoundButton currentButton;
	
    public SelectDeviceAdatper(List<WMDeviceInfo> infos, Context context) {
    	this.infos = infos;
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		if (convertView == null) {
			convertView = inflater.inflate(R.layout.device_channel_checkbox_item, parent, false);
		}
		WMDeviceInfo deviceInfo = getGroup(groupPosition);
		WMChannelInfo channel = getChild(groupPosition, childPosition);
		String channelName = Utils.isEmpty(channel.getChannelName()) ? "通道" + childPosition : channel.getChannelName(); 
